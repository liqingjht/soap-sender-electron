...
<p>6-5. 选择旧版本的根目录文件夹后，可以将历史记录导入到当前软件</p>
<p>6-6. 选择旧版本的根目录文件夹后，可以将 SOAP 列表导入到当前软件</p>
</li>
</ol>
<hr />
<p>历史更新记录：</p>
...
<ol>
<li>添加一些动画和效果</li>
<li>导出 PDF 或者 TXT 成功后自动打开文件夹，并选中导出的文件</li>
<li>更改全局颜色</li>
<li>解决发送失败后的出错问题</li>
<li>解决不能自动删除空白的 HTTP 头和 SOAP 参数</li>
<li>添加提交意见建议的功能</li>
<li>添加主页地址用来在没有 NTGR 路由器的时候使用主页地址来测试客户端</li>
<li>修改一些拼写错误和样式不统一的问题</li>
<li>解决 Method 和 Action 为空时，Response 页只显示个冒号的问题</li>
<li>在 Method 或 Action 变化时，清除 Response 信息</li>
<li>Response 为空时，pretty 和 copy 按钮前添加判断</li>
<li>优化时间轴显示</li>
<li>点击作者邮箱的时候，如果能连接到主页，就弹出提交意见对话框，如果连接不了，打开系统默认邮件应用</li>
</ol>
</blockquote>
...
<ol>
<li>在 PDF 报告上添加目录</li>
<li>解决历史纪录里颜色标注 Response Code 错误的问题</li>
<li>解决导出 PDF 报告时，如果多次不选择文件夹后再导出，会有多个提示的问题</li>
<li>解决显示 HTTP 包时，如果单行过长，不会自动换行的问题</li>
<li>添加导出历史记录到 TXT 文档里</li>
<li>添加通过解析 SOAP 请求字符串，将 HTTP 头、SOAP 参数等自动填写的功能</li>
<li>解决发送的 SOAP 包里 XML 里的格式问题</li>
</ol>
</blockquote>
...
<p>1.0.9 更新记录</p>
<ol>
<li>添加软件启动时检查是否有新版本，并使用系统通知的功能</li>
<li>添加 API Timeout 选项</li>
<li>添加使用 SOAPLogin API 登陆的方式</li>
<li>添加将历史记录导出成格式漂亮的 PDF 功能</li>
<li>添加清除所有历史记录功能</li>
<li>添加恢复默认设置的功能</li>
<li>添加从旧版本导入历史记录的功能</li>
<li>添加从旧版本导入 SOAP 列表功能</li>
<li>修复版本比较错误的问题</li>
<li>其他多处易用性的改动</li>
</ol>
</blockquote>
...
<p>1.0.3 更新记录</p>
<ol>
<li>板子信息里添加 LoginMethod 字段，但是还没有使用 SOAPLogin API认证</li>
<li>修复 Action 列表不能自动更新的问题</li>
<li>修复在历史纪录里点击 Resend 不能自动填写参数的问题</li>
<li>修复参数的值不能为空的问题</li>
<li>修复密码正确的时候没有提示的问题</li>
<li>更新 SOAP API 列表</li>
</ol>
</blockquote>
